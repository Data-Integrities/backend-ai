...
app.post('/start', async (req, res) => {
    ...
    try {
        ...
        const isSystemd = await fs.access('/etc/systemd/system').then(() => true).catch(() => false);
        ...
        try {
            ...
            const CORRELATION_FILE = path.join(AGENT_DIR, '.correlationId');
            await fs.writeFile(CORRELATION_FILE, correlationId, 'utf8');
...
app.listen(PORT, '0.0.0.0', async () => {
    ...
    if (!correlationId) {
        try {
            correlationId = (await fs.readFile('/tmp/ai-agent-manager.correlationId', 'utf8')).trim();
    ...
    try {
        const tempAgentName = (await fs.readFile('/tmp/ai-agent-manager.agentName', 'utf8')).trim();
