...
async function ensureReleasesDir() {
    try {
        await promises_1.default.mkdir(RELEASES_DIR, { recursive: true });
    }
    catch (error) {
...
const storage = multer_1.default.diskStorage({
    destination: async (req, file, cb) => {
        ...
        const versionDir = path_1.default.join(RELEASES_DIR, version);
        try {
...
`;
        // Write update script
        await promises_1.default.writeFile('/tmp/hub-update.sh', updateScript, { mode: 0o755 });
...
app.get('/version', async (req, res) => {
    try {
        const packagePath = path_1.default.join(HUB_DIR, 'package.json');
...
app.get('/releases/:version/:file', async (req, res) => {
    try {
        ...
        const filePath = path_1.default.join(RELEASES_DIR, version, file);
...
app.get('/releases', async (req, res) => {
    try {
        const versions = await promises_1.default.readdir(RELEASES_DIR);
        const releases = [];
        for (const version of versions) {
            const versionDir = path_1.default.join(RELEASES_DIR, version);
            const files = await promises_1.default.readdir(versionDir);
