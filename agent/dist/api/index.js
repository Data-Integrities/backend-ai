...
const capability_loader_1 = require("./capability-loader");
const shared_1 = require("@proxmox-ai-control/shared");
const ai_command_processor_1 = require("./ai-command-processor");
...
(async () => {
    try {
        ...
        const logManager = new shared_1.LogManager(__dirname, 'agent.log', retentionDays);
...
let CORRELATION_ID = process.argv[2];
if (CORRELATION_ID) {
    startupLog(1, `Starting with correlationId: ${CORRELATION_ID}`);
}
// Keep two copies for dual-memory approach
let CORRELATION_ID_FOR_CALLBACK = CORRELATION_ID;
let CORRELATION_ID_FOR_POLLING = CORRELATION_ID;
...
const capabilityLoader = new capability_loader_1.CapabilityLoader();
...
try {
    ...
    if (mainConfig?.anthropic?.apiKey) {
        ...
        aiProcessor = new ai_command_processor_1.AgentAICommandProcessor(mainConfig.anthropic.apiKey, capabilitiesManager);
...
app.post('/api/execute', async (req, res) => {
    const { command, requestId = (0, uuid_1.v4)(), async = false, correlationId } = req.body;
    if (!command) {
    ...
    if (async) {
        ...
        try {
            ...
            if (correlationId) {
                try {
                    await axios_1.default.post(`${HUB_URL}/api/executions/${correlationId}/fail`, {
                        error: error.message
                    });
                }
                catch (err) {
...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        ...
        const commandsToExecute = new Set();
...
async function executeCommand(parsed, correlationId) {
    switch (parsed.type) {
        case 'service':
...
async function executeCommandAsync(parsed, requestId, correlationId) {
...
const server = app.listen(PORT, '0.0.0.0', async () => {
    ...
    for (let i = 0; i < 10; i++) {
        try {
            const testResponse = await axios_1.default.get(`http://localhost:${PORT}/health`);
    ...
    if (CORRELATION_ID_FOR_CALLBACK) {
        ...
        try {
            await axios_1.default.post(`${HUB_URL}/api/executions/${CORRELATION_ID_FOR_CALLBACK}/complete`, {
