...
exports.setupCommandQueueEndpoints = setupCommandQueueEndpoints;
const command_queue_1 = require("./command-queue");
function setupCommandQueueEndpoints(app) {
    ...
    app.post('/api/commands/queue', (req, res) => {
        ...
        const command = command_queue_1.commandQueue.addCommand(agent, operation);
        res.json({
            success: true,
            command,
    ...
    app.get('/api/commands/pending', (req, res) => {
        const commands = command_queue_1.commandQueue.getPendingCommands();
        res.json({ commands });
    });
    ...
    app.post('/api/commands/:commandId/processing', (req, res) => {
        ...
        const success = command_queue_1.commandQueue.markProcessing(commandId, correlationId);
        if (success) {
    ...
    app.post('/api/commands/:commandId/complete', (req, res) => {
        ...
        const success = command_queue_1.commandQueue.markCompleted(commandId, result);
        if (success) {
            res.json({ success: true });
        }
        else {
    ...
    app.post('/api/commands/:commandId/fail', (req, res) => {
        ...
        const success = command_queue_1.commandQueue.markFailed(commandId, error || 'Unknown error');
        if (success) {
            res.json({ success: true });
        }
        else {
    ...
    app.get('/api/commands/:commandId', (req, res) => {
        const { commandId } = req.params;
        const command = command_queue_1.commandQueue.getCommand(commandId);
        if (command) {
            res.json(command);
        }
        else {
    ...
    app.post('/api/commands/execute', async (req, res) => {
        ...
        while (Date.now() - startTime < timeout) {
            ...
            const currentCommand = command_queue_1.commandQueue.getCommand(command.id);
            if (!currentCommand) {
        ...
        // Timeout
        command_queue_1.commandQueue.markFailed(command.id, 'Timeout waiting for browser execution');
        res.status(408).json({
            success: false,
...
//# sourceMappingURL=command-queue-endpoints.js.map