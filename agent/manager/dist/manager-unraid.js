...
(async () => {
    try {
        ...
        const managerLogManager = new shared_1.LogManager(__dirname, 'manager.log', retentionDays);
        ...
        const agentLogManager = new shared_1.LogManager(path_1.default.join(__dirname, '..'), 'agent.log', retentionDays);
...
async function getManagerVersion() {
    try {
        ...
        for (const location of locations) {
            try {
                const packageJson = JSON.parse(await promises_1.default.readFile(location, 'utf-8'));
...
async function isAgentRunning() {
    try {
        // First check pidfile
        if (await promises_1.default.access(AGENT_PIDFILE).then(() => true).catch(() => false)) {
            const pid = (await promises_1.default.readFile(AGENT_PIDFILE, 'utf-8')).trim();
...
app.post('/stop', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            await promises_1.default.unlink(AGENT_PIDFILE).catch(() => { });
        }
        catch {
